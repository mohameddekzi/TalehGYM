            ...
            {steps.map((s, i) => (
              <div key={s} className="flex items-center gap-2">
                <div className={`grid h-8 w-8 place-items-center rounded-full text-sm font-bold ${
                  i <= step ? "bg-brand-orange text-white" : "bg-ink-800 text-zinc-500"
                }`}>
                  {i < step ? <Check size={16} /> : i + 1}
                </div>
                <span className={`hidden text-sm sm:block ${i <= step ? "text-white" : "text-zinc-500"}`}>{s}</span>
                {i < steps.length - 1 ? <div className="h-px w-8 bg-ink-700 sm:w-12" /> : null}
              </div>
            ))}
          </div>
          ...
          <div className="card p-7">
                ...
                {plans.map((p) => (
                  ...
                  >
                    ...
                    <p className={`mt-3 font-display text-2xl font-extrabold ${accentText[p.accent]}`}>
                      ${p.monthly}<span className="text-sm font-normal text-zinc-500">/mo</span>
                    </p>
                  </button>
                ))}
              </div>
            ...
            {step === 2 && (
              <div className="space-y-4">
                <h3 className="font-display text-lg font-bold text-white">Review &amp; confirm</h3>
...
function Select({ label, options }: { label: string; options: string[] }) {
  return (
    <div>
      <label className="mb-1.5 block text-sm font-medium text-zinc-300">{label}</label>
      <select className="w-full rounded-xl border border-white/10 bg-ink-850 px-4 py-3 text-sm text-white focus:border-brand-orange/60 focus:outline-none">
        {options.map((o) => (
          <option key={o} className="bg-ink-850">{o}</option>
        ))}
      </select>
    </div>
  );
